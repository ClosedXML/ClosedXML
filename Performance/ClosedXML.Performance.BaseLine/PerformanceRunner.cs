...
#if PERF_BASELINE
namespace ClosedXML.Performance.BaseLine
#else
...
#endif
{
    public class PerformanceRunner : IPerformanceRunner
    {
                    ...
                    for (int j = 1; j <= columns; j++)
                    {
                        ...
                        else
                        {
                            sheet.Cell(i, j).FormulaA1 = string.Format("=SUM({0}$1:{0}{1})/SUM($A{1}:${2}{1})",
                                sheet.Column(j).ColumnLetter(), i - 1,
                                lastColumnLetter); // i.e. for K8 there will be =SUM(K$1:K7)/SUM($A7:$GR7)
                        }
                    }
                }
        ...
        public static void Run1220()
        {
            using (var wb = new XLWorkbook("e:\\ClosedXML\\1220_test.xlsx"))
            {
                var ws = wb.Worksheets.ToList();
            }

        }

        public static void Run1221()
        {
            using (var wb = new XLWorkbook(@"e:\ClosedXML\1221_input.xlsx"))
            {
                var ws = wb.Worksheets.ToList();

                foreach (var xlWorksheet in ws)
                {
                    Console.WriteLine($"Analyzing {xlWorksheet.Name}...");
                    foreach (var xlCell in xlWorksheet.CellsUsed())
                    {
                        /*foreach (var searchTerm in keyWordList)
                        {
                            var searchTermExtractedText = searchTerm.STRIPPED_EC_CIRCUIT.ToLower();

                            if (xlCell.Value.ToString().ToLower().Contains(searchTermExtractedText))
                            {
                                found = true;
                                //Do some work here
                            }
