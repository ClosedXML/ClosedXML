    ...
    internal static class OpenXmlHelper
    {
        ...
        private static XLColor ConvertToClosedXMLColor(IColorTypeAdapter openXMLColor, IDictionary<string, Color> colorCache)
        {
            ...
            if (openXMLColor != null)
            {
                ...
                {
                    var htmlColor = "#" + openXMLColor.Rgb.Value;
                    if (colorCache == null || !colorCache.TryGetValue(htmlColor, out var thisColor))
                    {
                        var sKColor = ColorStringParser.ParseFromHtml(htmlColor);
                        thisColor = sKColor.ConvertToOpenXmlColor();
