...
namespace ClosedXML.Excel
{
    public partial class XLWorkbook
    {
        ...
        private struct FontInfo { public UInt32 FontId; public XLFont Font; };
        private struct FillInfo { public UInt32 FillId; public XLFill Fill; };
        private struct BorderInfo { public UInt32 BorderId; public XLBorder Border; };
        private struct NumberFormatInfo { public UInt32 NumberFormatId; public XLNumberFormat NumberFormat; };
        ...
        private void CreateParts(SpreadsheetDocument document)
        {
            ...
            UInt32 sheetId = 0;
            foreach (var xlWorksheet in this.Worksheets)
            {
                sheetId++;
                WorksheetPart worksheetPart = workbookPart.AddNewPart<WorksheetPart>("rId" + sheetId.ToString());
                GenerateWorksheetPartContent(worksheetPart, xlWorksheet);
        ...
        private void GenerateWorkbookPartContent(WorkbookPart workbookPart1)
        {
            ...
            {
                sheetId++;
                Sheet sheet = new Sheet() { Name = xlWorksheet.Name, SheetId = (UInt32Value)sheetId, Id = "rId" + sheetId.ToString() };
                sheets.Append(sheet);
            }
        ...
        private void GenerateWorksheetPartContent(WorksheetPart worksheetPart, XLRange xlWorksheet)
        {
            Worksheet worksheet = new Worksheet();
            worksheet.AddNamespaceDeclaration("r", "http://schemas.openxmlformats.org/officeDocument/2006/relationships");
            SheetProperties sheetProperties = new SheetProperties() { CodeName = xlWorksheet.Name };

            UInt32 maxColumn = 0;
            UInt32 maxRow = 0;

            String sheetDimensionReference = "A1";
            if (xlWorksheet.Cells().Count() > 0)
            {
                maxColumn = xlWorksheet.Cells().Select(c => c.Address.Column).Max();
                maxRow = xlWorksheet.Cells().Select(c => c.Address.Row).Max();
                sheetDimensionReference = "A1:" + new XLAddress(maxRow, maxColumn).ToString();
            }
            
            SheetDimension sheetDimension = new SheetDimension() { Reference = sheetDimensionReference };

            Boolean tabSelected = xlWorksheet.Name == this.Worksheets.First().Name;
            ...
            var distinctRows = xlWorksheet.Cells().Select(c => c.Address.Row).Distinct();
            foreach (var distinctRow in distinctRows.OrderBy(r=>r))
            {
                Row row = new Row() { RowIndex = (UInt32Value)distinctRow, Spans = new ListValue<StringValue>() { InnerText = "1:" + maxColumn.ToString() } };
                foreach (XLRange opCell in xlWorksheet.Cells().Where(c => c.Address.Row == distinctRow).Select(c => c))
                {
                    var styleId = sharedStyles[opCell.Style.ToString()].StyleId;
                    Cell cell;
                    var dataType = opCell.DataType;
                    var cellReference = opCell.Address.ToString();
                    ...
                    else if (dataType == XLCellValues.SharedString)
                    {
                        cellValue.Text = sharedStrings[opCell.Value].ToString();
                    }
                    else
                    {
                        cellValue.Text = opCell.Value;
                    }
        ...
        private void GenerateSharedStringTablePartContent(SharedStringTablePart sharedStringTablePart)
        {
            ...
            this.Worksheets.ForEach(w => combined.AddRange(w.Cells().Where(c => c.DataType == XLCellValues.SharedString && c.HasValue).Select(c => c.Value).Distinct()));
        ...
        {
            var defaultStyle = this.WorkbookStyle;
            ...
            {
                Bold bold = new Bold() { Val = fontInfo.Font.Bold };
                Italic italic = new Italic() { Val = fontInfo.Font.Italic };
                Underline underline = new Underline() { Val = GetUnderlineValue(fontInfo.Font.Underline) };
                Strike strike = new Strike() { Val = fontInfo.Font.Strikethrough };
                ...
                Color color = new Color() { Rgb = fontInfo.Font.Color };
                FontName fontName = new FontName() { Val = fontInfo.Font.FontName };
                FontFamilyNumbering fontFamilyNumbering = new FontFamilyNumbering() { Val = fontInfo.Font.FontFamilyNumbering };
            ...
            foreach (var fillInfo in sharedFills.Values)
            {
                ...
                ForegroundColor foregroundColor = new ForegroundColor() { Rgb = fillInfo.Fill.PatternColor };
                BackgroundColor backgroundColor = new BackgroundColor() { Rgb = fillInfo.Fill.PatternBackgroundColor };
            ...
            foreach (var borderInfo in sharedBorders.Values)
            {
                ...
                Color leftBorderColor = new Color() { Rgb = borderInfo.Border.LeftBorderColor };
                ...
                Color rightBorderColor = new Color() { Rgb = borderInfo.Border.RightBorderColor };
                ...
                Color topBorderColor = new Color() { Rgb = borderInfo.Border.TopBorderColor };
                ...
                Color bottomBorderColor = new Color() { Rgb = borderInfo.Border.BottomBorderColor };
                ...
                Color diagonalBorderColor = new Color() { Rgb = borderInfo.Border.DiagonalBorderColor };
            ...
            foreach (var styleInfo in sharedStyles.Values)
            {
                ...
                Boolean applyFill = styleInfo.Style.Fill.PatternType == XLFillPatternValues.None;
                XLBorder opBorder = styleInfo.Style.Border;
                Boolean applyBorder = (
                       opBorder.BottomBorder != XLBorderStyleValues.None 
                    || opBorder.DiagonalBorder != XLBorderStyleValues.None 
                    || opBorder.RightBorder != XLBorderStyleValues.None
                    || opBorder.LeftBorder != XLBorderStyleValues.None 
                    || opBorder.TopBorder != XLBorderStyleValues.None);
                ...
                Alignment alignment = new Alignment() { Horizontal = GetHorizontalAlignmentValue(styleInfo.Style.Alignment.Horizontal), Vertical = GetVerticalAlignmentValue(styleInfo.Style.Alignment.Vertical), Indent = styleInfo.Style.Alignment.Indent, ReadingOrder = (UInt32)styleInfo.Style.Alignment.ReadingOrder, WrapText = styleInfo.Style.Alignment.WrapText, TextRotation = styleInfo.Style.Alignment.TextRotation, ShrinkToFit = styleInfo.Style.Alignment.ShrinkToFit, RelativeIndent = styleInfo.Style.Alignment.RelativeIndent, JustifyLastLine = styleInfo.Style.Alignment.JustifyLastLine };
            ...
            var defaultFormatId = sharedStyles.Values.Where(s => s.Style.ToString() == this.WorkbookStyle.ToString()).Single().StyleId;
